...
+ (NSString *)dateFromTimesTamp:(NSTimeInterval )timesTamp format:(NSString *)format
{
...
// 获取date是年的第几周
- (NSInteger )weekthOfYear{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    //每周的第一天从星期一开始
    [calendar setFirstWeekday:2];
    
    NSDateComponents *comps = [calendar components:NSCalendarUnitWeekOfYear fromDate:self];
...
+ (NSString *)NowtimeIsInBeginTime:(NSString *)begin
    ...
    NSDate *begintime = [format dateFromString:begin];
    NSDate *endtime = [format dateFromString:end];
    ...
    NSTimeInterval endtimes = [endtime timeIntervalSince1970] * 1000;
    
    if (nowtimes < begintimes) { // 未开始
        return @"-1";
    }else if (nowtimes > endtimes) { // 已结束
        return @"1";
    }else { //直播中
        return @"0";
    }
...
+ (NSArray *)getMonthFirstAndLastDayWith:(NSDate *)newDate {
